...
public class Oop_Homework1 {
    public static void main(String[] args) {
        ...
        System.out.print(">> Sınıftaki öğrenci sayısını girer misin? ");
        ogrenci_sayisi = scanner.nextInt();
        ogrenci_Sayisi_girildi = true;
        
        if(ogrenci_sayisi <= 0){
            System.out.print(">> Geçersiz öğrenci sayısı girdiniz.");
        }else{
        ...
        for(int i =0; i < ogrenci_sayisi; i++){
            System.out.println(">>Öğrenci-" + (i + 1) + "|" + "Kimlik: " + ogrenci_kimlikleri[i] +"|"+ " Ad: " + ogrenci_adlari[i] +"|"+ " Ara Sınav Notu: " + ogrenci_ara_sinav_notlari[i] +"|"+ " Final Notu: " + ogrenci_final_notlari[i]);
        }
    }
    else if (secim==3){
        ...
        double toplanmis_not = 0;
        double sinifin_ortalamasi = 0 ;
        int sayac = 0 ;
        for(int i=0; i<ogrenci_sayisi ; i++){
